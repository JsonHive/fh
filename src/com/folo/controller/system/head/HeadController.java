package com.folo.controller.system.head;
...
import com.folo.service.system.appuser.AppuserManager;
import com.folo.service.system.fhsms.FhsmsManager;
import com.folo.service.system.user.UserManager;
import com.folo.util.AppUtil;
import com.folo.util.Const;
import com.folo.util.Jurisdiction;
import com.folo.util.PageData;
import com.folo.util.SmsUtil;
import com.folo.util.Tools;
import com.folo.util.Watermark;
import com.folo.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public Object getList() {
		...
		try {
			...
			if(null != strWEBSOCKET && !"".equals(strWEBSOCKET)){
				String strIW[] = strWEBSOCKET.split(",folo,");
				if(strIW.length == 5){
	...
	public Object sendEmail(){
		...
		if(null != strEMAIL && !"".equals(strEMAIL)){
			String strEM[] = strEMAIL.split(",folo,");
			if(strEM.length == 4){
				if("yes".endsWith(isAll)){
					try {
						List<PageData> userList = new ArrayList<PageData>();
	...
	public ModelAndView goEditEmail() throws Exception{
		...
		if(null != strSMS1 && !"".equals(strSMS1)){
			String strS1[] = strSMS1.split(",folo,");
			if(strS1.length == 2){
		...
		if(null != strSMS2 && !"".equals(strSMS2)){
			String strS2[] = strSMS2.split(",folo,");
			if(strS2.length == 2){
		...
		if(null != strFWATERM && !"".equals(strFWATERM)){
			String strFW[] = strFWATERM.split(",folo,");
			if(strFW.length == 5){
		...
		if(null != strIWATERM && !"".equals(strIWATERM)){
			String strIW[] = strIWATERM.split(",folo,");
			if(strIW.length == 4){
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",folo,"+pd.getString("PORT")+",folo,"+pd.getString("EMAIL")+",folo,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",folo,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",folo,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",folo,"+pd.getString("fcontent")+",folo,"+pd.getString("fontSize")+",folo,"+pd.getString("fontX")+",folo,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",folo,"+pd.getString("imgUrl")+",folo,"+pd.getString("imgX")+",folo,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	...
	public ModelAndView saveSys3() throws Exception{
		...
		Tools.writeFile(Const.WEBSOCKET,pd.getString("WIMIP")+",folo,"+pd.getString("WIMPORT")+",folo,"+pd.getString("OLIP")+",folo,"+pd.getString("OLPORT")+",folo,"+pd.getString("FHsmsSound"));	//websocket配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
